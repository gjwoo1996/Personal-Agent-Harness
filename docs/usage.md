...
`setup.sh`는 기본 `rules`를 설치하고 verify를 실행합니다.
...
`update.sh`는 harness clone에서 `git pull --ff-only`, install, verify 순으로 실행합니다. 관리 파일은 `.harness/backups/<timestamp>/`에 백업됩니다.

### `Personal-Agent-Harness/` 폴더 유지

copy mode이므로 규칙 파일은 대상 프로젝트에 복사됩니다. 다만 `update.sh`가 harness clone에서 `git pull`을 실행하므로 프로젝트 안의 `Personal-Agent-Harness/` 폴더는 유지하는 것을 권장합니다.
